...
### 推荐配置：MiniMax VLM + Ollama bge-m3 Embedding

MineContext 使用两个 AI 模型协同工作：
- **视觉语言模型 (VLM)**：用于截图分析和理解
- **Embedding 模型**：用于语义搜索和上下文检索

推荐配置使用 MiniMax 作为 VLM，本地运行 Ollama bge-m3 模型作为 embedding。

#### 为什么推荐这个配置？

- **MiniMax VLM**：低成本且一流的视觉理解能力
- **Ollama bge-m3**：最先进的 embedding 模型，本地运行，支持多语言和多模态检索
...
这会下载约 1.5GB 的 bge-m3 embedding 模型。根据网络连接情况，首次运行可能需要几分钟。
...
你应该会收到一个包含 embedding 向量的 JSON 响应。
...
1. 选择 **MiniMax** 作为模型平台
2. 输入你的 MiniMax API key（从 https://platform.minimaxi.com/user-center/basic-information/interface-key 获取）
3. 选择 VLM 模型（如 MiniMax-M2.7）
4. **启用"使用自定义 embedding 模型"**
5. 填写 embedding 字段：
   - **模型名称**：`bge-m3`
   - **Base URL**：`http://localhost:11434/v1`
   - **API Key**：任意字符串（Ollama 不需要认证 — 使用 `ollama` 作为占位符）
6. 点击**保存**
...
**localhost:11434 连接被拒绝？**
- 确保 Ollama 正在运行
- 检查防火墙设置
- 尝试 `curl http://localhost:11434/api/tags` 测试连接
